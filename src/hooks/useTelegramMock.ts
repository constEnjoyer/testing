...
export function useTelegramMock(): void {
  useClientOnce(() => {
    // Если уже замокано и мы в TMA - ничего не делаем
    if (getFromSessionStorage(STORAGE_KEYS.ENV_MOCKED, null) && isTMA('simple')) {
      return;
    }
    ...
    saveToSessionStorage(STORAGE_KEYS.ENV_MOCKED, '1');
    mockTelegramEnv(lp);
    console.warn(
